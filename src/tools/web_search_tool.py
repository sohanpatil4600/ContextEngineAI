...
class TavilySearchTool(BaseTool):
    ...
    description: str = "Search the web for recent information and developments on research topics using Tavily"
        ...
        try:
            if not self.api_key:
                return json.dumps({
                    "status": "ERROR",
                    "source_used": "WEB",
                    "answer": "Web search unavailable - Tavily API key not configured.",
                    "citations": [],
                    "confidence": 0.0,
                    "error": "Missing API key"
                })
