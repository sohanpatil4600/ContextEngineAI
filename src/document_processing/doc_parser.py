...
class TensorLakeClient:
    ...
    def list_uploaded_files(self):
        try:
            ...
            headers = {"Authorization": f"Bearer {self.doc_ai.api_key}"}
            response = requests.get(url, headers=headers)
            response.raise_for_status()
            data = response.json()
            items = data.get("items", [])
            print(f"TensorLake files found: {len(items)}")
            
            # Create dummy objects to maintain compatibility
            class DummyFile:
                def __init__(self, id, name, size, mime):
                    self.id = id
                    self.name = name
                    self.file_size = size
                    self.mime_type = mime
            ...
            for item in items:
                ...
                print(f"  - {f.name} (ID: {f.id}, Size: {f.file_size} bytes, Type: {f.mime_type})")
                files.append(f)
            return files
    ...
    def upload(self, paths: Iterable[str]) -> List[str]:
        ...
        for path in paths:
            ...
            try:
                ...
                with open(path, "rb") as f:
                    response = requests.post(url, headers=headers, files={"file": (os.path.basename(path), f)})
                    response.raise_for_status()
                    fid = response.json().get("file_id")
                    if not fid:
                        raise Exception("No file_id found in response")
        ...
        before_ids = [f.id for f in files_before]
        new_files = [f for f in files_after if f.id not in before_ids]
        if new_files:
